...
classdef QuadMeshFrom3d < QuadMesh
    ...
    methods
        function obj = QuadMeshFrom3d(args)
            arguments
                ...
                args.dnum_parallel = 1
                args.dnum_orthogonal = 1
            end
            % ---
            obj = obj@QuadMesh;
            % ---
    ...
    methods (Access = private)
        ...
        function obj = build(obj)
            % ---
            parlin_1 = obj.parallel_line_1;
            parlin_2 = obj.parallel_line_2;
            vec1 = parlin_1(1,:) - parlin_1(2,:);
            vec2 = parlin_2(1,:) - parlin_2(2,:);
            len1 = norm(vec1);
            len2 = norm(vec2);
            % ---
            ...
            % ---
            if dot(vec1,vec2) < 0
                parlin_2 = parlin_2(end:-1:1,:);
            end
            % ---
            ms_p = obj.dnum_parallel;
            ms_o = obj.dnum_orthogonal;
            node = zeros(3,(ms_p+1) * (ms_o+1));
            elem = zeros(4,ms_p * ms_o);
            % ---
            for i = 1:ms_o+1
                id_node = (i-1) * (ms_p+1) + (1:ms_p+1);
                node(1,id_node) = 
            end
