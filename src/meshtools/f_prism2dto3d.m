function mesh = f_prism2dto3d(dom2d,layer)
...
for k=1:nbLayers	% k : current layer
    ...
    % upper face
    elem(4,nt+1:nt+nbElem2D) = dom2d.mesh.elem(1,:) + nbNode2D * k;
    elem(5,nt+1:nt+nbElem2D) = dom2d.mesh.elem(2,:) + nbNode2D * k;
    elem(6,nt+1:nt+nbElem2D) = dom2d.mesh.elem(3,:) + nbNode2D * k;
    % zone 3D
    % elem(7,nt+1:nt+nbElem2D) = dom2D.mesh.elem(4,:) + k * pi;
    elem(7,nt+1:nt+nbElem2D) = dom2d.mesh.elem(4,:) + codeLayer(k);
    % zone 2D
    elem(8,nt+1:nt+nbElem2D) = dom2d.mesh.elem(4,:);
    % layer id
    elem(9,nt+1:nt+nbElem2D) = k;
    % go to the next zone
    nt=nt+nbElem2D;
end
...
mesh = f_mdsprism(node,elem,'full');
