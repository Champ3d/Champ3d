...
%%
figure
plot(dataAN.xline, dataAN.A_01(2,:), 'b', 'DisplayName', '(AN) A at z coil1'); hold on
plot(dataAN.xline, dataAN.A_02(2,:), 'r', 'DisplayName', '(AN) A at z coil2'); hold on
plot(px, AFEM_01, "bo", "LineWidth", 2, "DisplayName", "(FEM2d) A at z coil1"); hold on
plot(px, AFEM_02, "ro", "LineWidth", 2, "DisplayName", "(FEM2d) A at z coil2"); hold on
legend;

return
%%
figure
id_dom_ = fieldnames(WPT_CirCoil.mesh.dom);
for i = 1:length(id_dom_)
    id_dom = id_dom_{i};
    WPT_CirCoil.mesh.dom.(id_dom).plot('face_color',f_color(i),'edge_color','non');
end
hold on;
plot(px, py, 'ro');
...
% ---
figure
f_quiver(WPT_CirCoil.mesh.celem,real(WPT_CirCoil.field.b));
view(2)
axis tight

% ---
figure
f_quiver(WPT_CirCoil.mesh.celem,imag(WPT_CirCoil.field.b));
view(2)
axis tight

% ---
figure
f_contour('node',WPT_CirCoil.mesh.node,'scalar_field',real(WPT_CirCoil.dof.a),'nb_level',20);
% ---
figure
f_contour('node',WPT_CirCoil.mesh.node,'scalar_field',abs(imag(WPT_CirCoil.dof.a)),'nb_level',20);
...
idElem_Txfer = WPT_CirCoil.mesh.dom.Tx_fer.gindex; % d_2
idElem_Rxfer = WPT_CirCoil.mesh.dom.Rx_fer.gindex; % d_3

Relem = WPT_CirCoil.mesh.celem(1,:).';
Belem = WPT_CirCoil.field.b;

Bxele = abs(Belem(:,1));
Byele = abs(Belem(:,2));
Selem = WPT_CirCoil.mesh.velem;
Velem = (2*pi*Relem) .* Selem;

% --- Losses
alpha = 1.32;
beta = 2;
k = 6.47;
%pv_core = k * fr^alpha * Brm^beta;

PlossTxcore = sum( (k * fr^alpha) .* Bxele(idElem_Txfer) .^beta .* Velem(idElem_Txfer) + ...
                   (k * fr^alpha) .* Byele(idElem_Txfer) .^beta .* Velem(idElem_Txfer) );
PlossRxcore = sum( (k * fr^alpha) .* Bxele(idElem_Rxfer) .^beta .* Velem(idElem_Rxfer) + ...
                   (k * fr^alpha) .* Byele(idElem_Rxfer) .^beta .* Velem(idElem_Rxfer) );
...
WPT_CirCoil.field.pv = zeros(1,WPT_CirCoil.mesh.nb_elem);
% ---
id_elem = WPT_CirCoil.dom.Tx_fer.meshdom.gindex;
WPT_CirCoil.field.pv(id_elem) = ...
    (k * fr^alpha) .* Bxele(id_elem) .^beta + ...
    (k * fr^alpha) .* Byele(id_elem) .^beta;
% ---
id_elem = WPT_CirCoil.dom.Rx_fer.meshdom.gindex;
WPT_CirCoil.field.pv(id_elem) = ...
    (k * fr^alpha) .* Bxele(id_elem) .^beta + ...
    (k * fr^alpha) .* Byele(id_elem) .^beta;
% ---
Tx_pv_density = zeros(1,Tx_nb_turn);
for i = 1:Tx_nb_turn
    id_dom = ['Tx_coil_turn_' num2str(i)];
    % ---
    WPT_CirCoil.dom.(id_dom).get_quantity;
    % ---
    id_elem = WPT_CirCoil.dom.(id_dom).meshdom.gindex;
    WPT_CirCoil.field.pv(id_elem) = ...
        WPT_CirCoil.dom.(id_dom).quantity.loss_density;
    Tx_pv_density(i) = WPT_CirCoil.dom.(id_dom).quantity.loss_density;
end
% ---
Rx_pv_density = zeros(1,Rx_nb_turn);
for i = 1:Rx_nb_turn
    id_dom = ['Rx_coil_turn_' num2str(i)];
    % ---
    WPT_CirCoil.dom.(id_dom).get_quantity;
    % ---
    id_elem = WPT_CirCoil.dom.(id_dom).meshdom.gindex;
    WPT_CirCoil.field.pv(id_elem) = ...
        WPT_CirCoil.dom.(id_dom).quantity.loss_density;
    Rx_pv_density(i) = WPT_CirCoil.dom.(id_dom).quantity.loss_density;
end

%%

figure
WPT_CirCoil.dom.Tx_fer.plotpv; hold on
WPT_CirCoil.dom.Rx_fer.plotpv; hold on
for i = 1:Tx_nb_turn
    id_dom = ['Tx_coil_turn_' num2str(i)];
    WPT_CirCoil.dom.(id_dom).plotpv; hold on
end
% ---
for i = 1:Rx_nb_turn
    id_dom = ['Rx_coil_turn_' num2str(i)];
...
figure
plot(Tx_pv_density,'DisplayName','Tx out -> in'); hold on
plot(Rx_pv_density,'DisplayName','Rx out -> in'); legend;
